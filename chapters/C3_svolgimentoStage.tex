...
\begin{itemize}
    ...
    \item \textbf{Applicazione di DevOps}: ho raccolto tutte le attività legate alle fasi di \gls{DevOps} identificate, sviluppate ed integrate con i progetti Power Automate e Power Apps. Dopo aver esplorato tali tecnologie, era obiettivo aziendale comprendere se tale metodologia fosse a loro applicabile e con quali modalità.
    \item \textbf{Visione d'insieme}:  ho raccolto tutti i risultati ottenuti qualitativamente e quantitativamente.
\end{itemize}
...
\begin{itemize}
    \item \emph{Agile} e \emph{Scrum}: descrizione delle metodologie \emph{Agile} e \emph{Scrum}, spiegazione dei ruoli necessari e delle cerimonie previste. 
...
Essi sono direttamente accessibili dalla \emph{home} di Power Automate e sono divisi in moduli testuali corredati da immagini, dalla durata e argomenti specifici. 
...
\noindent Ad ogni Trigger possono essere collegate, in serie o in parallelo, una moltitudine di Azioni, ciascuna responsabile di uno specifico compito.
Per esempio sono presenti le azioni "Inizializza variabile", "Avvia e attendi un'approvazione", "Teams - Crea una chat" e "Outlook - Invia un messaggio di posta elettronica".\\
Sono inoltre presenti azioni dedicate alla gestione logica dei flussi come "Condizione" e "Do until".\\
La prima rappresenta la struttura di controllo rappresentata nei classici linguaggi di programmazione con "\emph{If}", responsabile della ramificazione dell'esecuzione del flusso in base a una condizione specifica.
La seconda rappresenta la struttura di controllo rappresentata nei classici linguaggi di programmazione con "\emph{Do while}", responsabile della ripetizione condizionata di un insieme di azioni garantendone sempre la prima esecuzione.
\begin{figure}[htbp] 
    \centering 
...
\end{figure}
\newline \noindent Successivamente ho individuato, durante un \emph{meeting} con il \emph{tutor} aziendale, la necessità di integrare i flussi Power Automate con il \emph{software} gestionale \hyperref[WOW]{WOW} e gli altri prodotti aziendali.\\
...
Dopo aver compreso la tecnologia descritta, ho realizzato un primo flusso al fine di testare l'utilizzo di: Trigger, Azioni, collegamento con i servizi Microsoft OneDrive ed Excel, Azioni condizionali, cicli \emph{Do until}, inizializzazione e modifica di variabili.\\
Esso viene eseguito automaticamente quando viene creato un \emph{file} dentro ad un \emph{path} specifico su OneDrive e di conseguenza invia una richiesta di approvazione, notificata sia tramite Teams che Outlook.
...
\begin{figure}[htbp] 
    \centering 
    \includegraphics[width=0.6\columnwidth]{flussoApprovazioni} 
...
Essa esclude il controllo della presenza di un \emph{file} omonimo e quindi non include il ciclo \emph{Do until} e l'inizializzazione e modifica di variabili.\\
...
Egli è il responsabile in azienda di tutti i loro prodotti realizzati utilizzando le tecnologie oggetto del mio \emph{stage}.
Assieme le abbiamo attentamente analizzate e modificate al fine di migliorarle ed applicarci le soluzioni da me individuate relativamente alle pratiche \gls{DevOps} richieste.\\\\
...
Le Soluzioni preservano i loro dati, e quelli degli elementi in esse contenuti, nel sistema di archiviazione Microsoft Dataverse, il quale svolge il ruolo di \emph{database} centralizzato per applicazioni sviluppate con la Power Platform.\\
Tale adozione permette di usufruire di funzionalità altrimenti assenti, tra cui la possibilità di poter visionare la "Version history" dei flussi Power Automate. Tale funzionalità è in grado di ripristinare specifiche versioni precedenti dei flussi, agevolando le fasi di sviluppo.
\begin{figure}[htbp] 
    \centering 
    \includegraphics[width=0.7\columnwidth]{versionHistory} 
...
\begin{itemize}
    ...
    Ho successivamente scartato tali soluzioni a causa degli eccessivi tempi di esecuzione e ad altre limitazioni incontrate, come l'impossibilità per SharePoint di eseguire il comando di ricerca per sottostringa.
...
\noindent Le mie attività di sviluppo sulle applicazioni "Registro accessi" e "Non conformità" sono legate ad aspetti grafici e all'individuazione di possibili risoluzioni di problemi legati alle singole funzioni Fx. 
...
Prima di poter progettare una soluzione atta ad applicare le fasi di \gls{DevOps} a progetti realizzati con Power Automate e Power Apps, ho analizzato e compreso quali fossero le implicazioni di ogni \emph{step}.\\
...
\begin{figure}[htbp] 
    \centering 
    \includegraphics[width=0.6\columnwidth]{githubAppPermessi} 
...
Questo al fine di sostituire le logiche, realizzate tramite complessi \emph{scirpt}, responsabili per l'integrazione e la sincronizzazione tra le due piattaforme di pianificazione.\\
...
Ho quindi adattato, prima in un \emph{branch} di prova e poi nei restanti, il \emph{repository} Git dell'applicazione aziendale "Report di visita" in modo da rispettare le norme definite.
...
\begin{figure}[htbp] 
    \centering 
    \includegraphics[width=0.5\columnwidth]{organizzazioneRepo} 
...
Inoltre ho redatto i documenti “Norme di versionamento” e “Analisi statica” e li ho condivisi con il \emph{team} di sviluppo e con il \emph{\emph{tutor}} aziendale.\\  
...
\begin{itemize}
    ...
    \item Controlla l'eventuale presenza di un'omonima Solution già presente nel \emph{repository} e, se presente, la elimina 
    \item Carica il pacchetto relativo alla nuova versione della Solution nel \emph{repository}
\end{itemize}
Ho redatto il documento “Guida Jenkins”, il quale comprende tutte le nozioni e le norme relative all'adozione di Jenkins, al fine di applicare la fase \emph{Build} ai progetti Power Automate e Power Apps e l'ho condiviso con il \emph{team} di sviluppo e con il \emph{tutor} aziendale.\\\\
...
\noindent A causa della natura dei \emph{file} generati dalla fase \emph{Build} è emerso, similmente all'analisi statica del codice, la necessità di utilizzare principalmente gli strumenti nativi forniti da Microsoft per eseguire \emph{test} dinamici.\\
...
Al fine di applicare \emph{test} dinamici ai flussi tramite \emph{script}, ho individuato la possibilità di utilizzare i Trigger e le Azioni Power Automate legate all'utilizzo delle chiamate \gls{http}.
...
\subsubsection*{\emph{Test} su Power Apps}
...
\begin{figure}[htbp] 
    \centering 
    \includegraphics[width=1\columnwidth]{testStudio} 
...
Ho poi condiviso tali documenti con il \emph{team} di sviluppo e con il \emph{tutor} aziendale.\\
...
\begin{figure}[htbp] 
    ...
    \caption{Vista parziale dell'interfaccia atta a monitorare un flusso.}
...
Lo studio da me svolto durante lo \emph{stage} ha fatto emergere l'effettiva possibilità di applicare le metodologie \gls{DevOps} a progetti realizzati con Power Automate e Power Apps.\\\\
...
\begin{longtable}{|p{9cm}|c|}
    ...
    \hline Flusso per il \emph{retrieve} dei dati da SQL \emph{server} & 4 Azioni\\
